...
export default function MentorBiodata() {
  ...
  const { mutate, isLoading } = useMutation({
    ...
    //   builder
    //     .use()
    //     .mentor.update_mentor(base64decode(auth as string), mentorForm.values),
    // mutationKey: builder.mentor.update_mentor.use(
    //   base64decode(auth as string),
    //   mentorForm.values
    // ),

    mutationFn: () => builder.use().mentor.update_mentor(base64decode(auth as string), mentorForm.values),
  ...
  return (
    <form
      ...
      onSubmit={mentorForm.onSubmit(() => mutate())}
    >
      ...
      <div className="flex flex-col gap-7">
        ...
        <Button type="submit" loading={isLoading} classNames={classes}>
          Proceed
        </Button>
        {/* </Link> */}
